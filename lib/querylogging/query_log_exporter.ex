defmodule Querylogging.QueryLogExporter do
  ...
  def export_to_csv(query_logs) do
    ...
    unless File.exists?(@csv_file) do
      headers = ["query_string", "execution_time", "created_at", "updated_at"]
      write_to_csv(headers)
    end

    # Format data for CSV
    formatted_logs = Enum.map(query_logs, fn log ->
      [log.query_string, log.execution_time, log.created_at, log.updated_at]
    end)
